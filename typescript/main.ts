...
function insertTextAt(text: string, newText: string, pos: number, eraseAmount = 0) {
...
patterns.addEventListener("keydown", (ev) => {
  if(patterns.selectionStart == patterns.selectionEnd && !(patterns.value[patterns.selectionStart] && patterns.value[patterns.selectionStart] != "\n")) {
    if(ev.key == "Tab") {
      ...
      if(lastLine.endsWith("Introspection") || lastLine.endsWith("{")) leadingSpaces += 2;
      ...
      ev.preventDefault();
      if(forceAutocompleteIndex == -1) forceAutocompleteIndex = 1;
      else forceAutocompleteIndex++;
    }
  }
  setTimeout(() => {  // queue it for AFTER the event resolves
    handleInput();
    //@ts-expect-error
